...
import { createQuote, updateQuote, QuoteFormData, QuoteWithItems } from '@/lib/quotes';
...
export default function QuoteForm({ quote, onSuccess }: QuoteFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      expiry_date: quote?.expiry_date || defaultExpiryDate,
      notes: quote?.notes || '',
      terms_conditions: quote?.terms_conditions || 'Devis valable 30 jours. Conditions générales de vente disponibles sur demande.',
  ...
  const onSubmit = async (data: QuoteFormInputs) => {
    ...
    try {
      if (isEditMode) {
        ...
        const quoteId = await createQuote(user.id, data);
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-secondary-600">
            {isEditMode ? 'Modifiez les informations de votre devis' : 'Créez un nouveau devis'}
          </p>
        </div>
      </div>
